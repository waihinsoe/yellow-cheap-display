...
void loop() {
  ...
  if(millis()- lastImageChangeTime >= 3000) {
    if(isSunny) {
        lv_img_set_src(objects.weather_logo,&img_rain);
    }else{
      lv_img_set_src(objects.weather_logo, &img_sunny);
    }
    isSunny = !isSunny;
...
char temp_text[10];  // Array to hold the temperature text
char humi_text[10];  // Array to hold the humidity text
char pressure_text[10];  // Array to hold the pressure text

// Generate random values and set as text for labels
sprintf(temp_text, "%d", random(20, 26));  // Convert integer to string
lv_label_set_text(objects.temp_text, temp_text);  // Set the label text

sprintf(humi_text, "%d", random(80, 90));  // Convert integer to string
lv_label_set_text(objects.humi_text, humi_text);  // Set the label text

sprintf(pressure_text, "%d", random(1000, 1050));  // Convert integer to string
lv_label_set_text(objects.pressure_text, pressure_text);  // Set the label text
